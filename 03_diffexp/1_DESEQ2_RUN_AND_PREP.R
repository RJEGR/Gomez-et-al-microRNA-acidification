...
## --- paths centralised in config.R (were absolute ~/Documents/MIRNA_HALIOTIS/...) ---
...
path <- paste0(DATA_DIR, "/")   # all inputs for this module live under data/

count_f <- list.files(path = path, pattern = "^Counts\\.txt$",         full.names = TRUE)
DB_f    <- list.files(path = path, pattern = "^RNA_LOCATION_DB\\.tsv$", full.names = TRUE)
MTD_f   <- list.files(path = path, pattern = "^METADATA\\.tsv$",        full.names = TRUE)
